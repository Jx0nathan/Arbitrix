package io.arbitrix.core.strategy.pure_market_making.data;
 ...
 */
@Log4j2
@Component("pureMarketMakingSpotOrderTradeDataManager")
@ExecuteStrategyConditional(executeStrategyName = "pure_market_making")
public class PureMarketMakingSpotOrderTradeDataManager extends AbstractOrderTradeDataManager implements OrderTradeUpdateListener {
    ...
    public PureMarketMakingSpotOrderTradeDataManager(BinanceOpenOrderRunner binanceOpenOrderRunner,
            ...
            for (Order order : orders) {
                ...
                if (cacheOrder != null && cacheOrder.getPrice() != null) {
                    ...
                    if (isDifferentPrice(cacheOrder.getPrice(), order.getPrice())) {
                        ...
                        log.info("PureMarketMakingSpotOrderTradeDataManager.getCancelOrderListBaseOnBestPrice order price is same,exchange:{},orderPrice:{}", exchangeNameEnum.getValue(), order.getPrice());
                    }
                } else {
                    log.info("PureMarketMakingSpotOrderTradeDataManager.getCancelOrderListBaseOnBestPrice cacheOrder price is null");
                }
            }
    ...
    private List<Order> processOrderPollClear(List<Order> orderList) {
        ...
        if (cacheOrder != null && isDifferentPrice(cacheOrder.getPrice(), order.getPrice())) {
            ...
            if (Objects.nonNull(orderTradeUpdateEvent)) {
                log.info("PureMarketMakingSpotOrderTradeDataManager.processOrderPollClear remove cacheKey:{},orderTradeUpdateEvent:{}", cacheKey, JacksonUtil.toJsonStr(orderTradeUpdateEvent));
    ...
    public void orderTradeUpdateEvent(String exchangeName, OrderTradeUpdateEvent event) {
        ...
        if (event.getExecutionType() == ExecutionType.TRADE || event.getExecutionType() == ExecutionType.CANCELED) {
            ...
            if (event.getOrigClientOrderId().equals(cacheOrder.getOrigClientOrderId())) {
     ...
     * 仅将最新的订单放入缓存池
     *
     * @param exchangeName 交易所的名字
     * @param orderList    当前用户已存在的订单
     */
    public void updateOrderTradePool(String exchangeName, List<Order> orderList) {
        if (orderList == null || orderList.isEmpty()) {
            return;
        }
        Map<String, List<Order>> symbolMap = orderList.stream().collect(Collectors.groupingBy(Order::getSymbol));
        for (Map.Entry<String, List<Order>> entry : symbolMap.entrySet()) {
            List<Order> orderEntryList = entry.getValue();
            if (orderEntryList == null || orderEntryList.isEmpty()) {
                continue;
            }
            orderEntryList.sort(Comparator.comparing(Order::getTime).reversed());
            // 这里是上次运行结束后,最后挂的一个订单,
            Order bestOrder = orderEntryList.get(0);
            String cacheKey = OrderTradeUtil.buildOrderTradeKey(exchangeName, bestOrder.getSymbol(), bestOrder.getSide());
            orderTradePool.put(cacheKey, this.convertOrderToOrderTradeUpdateEvent(bestOrder));
        }
    }

    private OrderTradeUpdateEvent convertOrderToOrderTradeUpdateEvent(Order order) {
        ...
        orderTradeUpdateEvent.setSymbol(order.getSymbol());
        orderTradeUpdateEvent.setSide(order.getSide());
        orderTradeUpdateEvent.setEventTime(order.getTime());
        orderTradeUpdateEvent.setPrice(order.getPrice());
        orderTradeUpdateEvent.setOriginalQuantity(order.getOrigQty());
        return orderTradeUpdateEvent;
    }

    public void putNewOrderIntoOrderPool(String exchangeName, ExchangeOrder sportOrder) {
        OrderTradeUpdateEvent orderTradeUpdateEvent = this.convertNewOrderParamsPlaceResultToEvent(sportOrder);
        ...
        if (cacheOrder != null) {
            ...
            if (orderTradeUpdateEvent.getEventTime() > cacheOrder.getEventTime()) {
                ...
                log.warn("AccountDataManager.putNewOrderIntoOrderPool {} cacheOrder is not null why?", cacheKey);
            }
        } else {
            orderTradePool.put(cacheKey, orderTradeUpdateEvent);
        }
