...
public class OkxPrivateWebsocketHandler02 implements OkxPrivateWebsocketLogic {
    private final LoginRequestSign loginRequestSign;
    private final OkxOrderStatusStreamer okxBookTickerStreamer;
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        log.info("OkxPrivateWebsocketHandler02.afterConnectionEstablished is: {}", session);
        LoginRequest loginRequest = loginRequestSign.sign(PRIORITY);
        String loginJson = JacksonUtil.toJsonStr(loginRequest);
        log.info("OkxPrivateWebsocketHandler02.loginJson is: {}", loginJson);
    ...
    public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
        ...
        log.info("OkxPrivateWebsocketHandler02.handleMessage.session is {}  message is {}", session, message.getPayload());
        ...
        if (wssResponse.has(EVENT) && EVENT_LOGIN.equals(wssResponse.get(EVENT).asText())) {
            ...
            if (LOGIN_SUCCESS.equals(wssResponse.get(CODE).asText())) {
                ...
                log.warn("OkxPrivateWebsocketHandler02.Login.failed: {}", wssResponse);
            }
        ...
        if (isOrderFromOrderChannel(wssResponse)) {
            ...
            okxBookTickerStreamer.startOrderStatusStreaming(okxSubscribeOrderInfo);
        }
    }

    @Override
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        log.info("OkxPrivateWebsocketHandler02.handleTransportError session: {}", session, exception);
    }

    @Override
    public void afterConnectionClosed(WebSocketSession session, CloseStatus closeStatus) throws Exception {
        log.info("OkxPrivateWebsocketHandler02.afterConnectionClosed session: {}. closeStatus: {}", session, closeStatus);
    }
